var IPAQuiz = (function() {
	...
	var setup = function(div) {
		...
		var playCharacter = $('<div class="play"><audio id="sound" src='+charsToLinks[quizCharacter]+'></audio><div><button id = "button" class= "btn btn-info" onclick='+order+'>Play Sound</button></div></div>')
		var wordBox = $('<input class = "wordInput" placeholder = "Input word here"></input>');
		var button = $('<button id = "submit" class = "btn btn-success">Submit</button>');
		var feedback = $('<div class="feedback"></div>')
		var newChar = $('<button id = "new" class = "btn btn-info">New Sound</button>')

		div.append(header, character, playCharacter, wordBox, button, feedback, newChar);

		$('#new').on('click', function() {
			quizCharacter = chars[Math.floor(Math.random()*chars.length)];
			$('.sound').html("Sound: "+charsToCodes[quizCharacter])
			$('.play').html('<audio id="sound" src='+charsToLinks[quizCharacter]+'></audio><div><button id = "button" class= "btn btn-info" onclick='+order+'>Play Sound</button></div>')
		})
